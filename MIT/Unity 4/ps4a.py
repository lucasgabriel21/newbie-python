...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:  # As long as there are still letters left in the hand:
        ...
        displayHand(hand)
        word = input('Enter word, or a "." to indicate that you are finished: ').lower()  # Ask user for input
...
def playGame(wordList):
    ...
    """
    # ask the input of 'n', 'r' or 'e'
    n = HAND_SIZE
    while True:
        inp = input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ').lower()
        if inp == 'n':  # plays the new hand
            hand = dealHand(n)
            playHand(hand, wordList, n)
            print()
        elif inp == 'r': # if inputs 'r' for replay in the first try ask to play a new hand
            try:
                playHand(hand, wordList, n)
                print()
            except NameError:
                ...
                print()
        elif inp == 'e':  # only condition to exit the loop
            break
        else:
